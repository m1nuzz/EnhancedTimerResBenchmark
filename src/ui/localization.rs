...
impl Localization {
    ...
    pub fn get_measurement_stats(&self, mean: f64, p95: f64, mad: f64, outliers: usize) -> String {
        match self.language {
            Language::English => format!("       Ој={:.4} ms, p95={:.4} ms, MAD={:.4} ms, outliers={}", mean, p95, mad, outliers),
            Language::Ukrainian => format!("       Ој={:.4} РјСЃ, p95={:.4} РјСЃ, MAD={:.4} РјСЃ, РІРёРєРёРґС–РІ={}", mean, p95, mad, outliers),
            Language::Russian => format!("       Ој={:.4} ms, p95={:.4} ms, MAD={:.4} ms, РІС‹Р±СЂРѕСЃРѕРІ={}", mean, p95, mad, outliers),
            Language::Chinese => format!("       Ој={:.4} жЇ«з§’, p95={:.4} жЇ«з§’, MAD={:.4} жЇ«з§’, еј‚еёёеЂј={}", mean, p95, mad, outliers),
        }
    }
